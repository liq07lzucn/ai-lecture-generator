...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .result{display:none;margin-top:30px;padding:20px;background:#f8f9fa;border-radius:12px}
...
<body>
    <div class="container">
        ...
        <div class="history-bar">
            <span class="history-count" id="history-count">📚 加载中...</span>
    ...
    <script>
        fetch('/api/history').then(r=>r.json()).then(data=>{
            ...
            let h='';
            data.forEach((d,i)=>{
                h+='<div class="history-item"><strong>'+(i+1)+'. '+d.topic+'</strong><br><small style="color:#888;">📅 '+d.time+'</small><br><a href="'+d.lecture_url+'" target="_blank" style="color:#667eea;">📄 讲义</a>'+(d.audio_url?' | <a href="'+d.audio_url+'" target="_blank">🔊 语音</a>':'')+'</div>';
            });
        ...
        function generate(e){e.preventDefault();document.getElementById('loading').style.display='block';fetch('/api/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({topic:document.getElementById('topic').value,content:document.getElementById('content').value,duration:parseInt(document.getElementById('duration').value),style:document.getElementById('style').value})}).then(r=>r.json()).then(data=>{document.getElementById('loading').style.display='none';if(data.error){document.getElementById('result').innerHTML='<p style="color:red;">❌ '+data.error+'</p>';}else{document.getElementById('result').innerHTML='<h3>✅ 完成!</h3><p>讲义：<a href="'+data.lecture_path+'">'+data.lecture_path+'</a></p>'+(data.audio_path?'<p>语音：<a href="'+data.audio_path+'">播放</a></p>':'');}document.getElementById('result').style.display='block';});}
    </script>
</body>
</html>'''
    ...
    for meta_file in sorted(output_dir.glob("meta_*.json"), key=lambda x: x.stat().st_mtime, reverse=True):
        try:
            with open(meta_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            lecture_path = data.get('lecture_path', '')
            audio_path = data.get('audio_path', '')
            history.append({
                'topic': data.get('topic', ''),
                'time': datetime.fromtimestamp(meta_file.stat().st_mtime).strftime('%Y-%m-%d %H:%M'),
                'lecture_url': '/file/' + Path(lecture_path).name if lecture_path else '',
                'audio_url': '/file/' + Path(audio_path).name if audio_path else ''
            })
        except:
            pass
...
def api_generate():
    ...
    try:
        ...
        result = generator.generate(source=source, topic=data.get('topic',''), duration=data.get('duration',10), style=data.get('style','通俗易懂'))
